...
// TestExtractTarRejectsOversizedEntryAndCleansDirectory 验证条目超限时不会保留部分解压内容。
func TestExtractTarRejectsOversizedEntryAndCleansDirectory(t *testing.T) {
	tempDir := t.TempDir()
	tarPath := filepath.Join(tempDir, "oversized.tar")
	extractDir := filepath.Join(tempDir, "extract")
	file, err := os.Create(tarPath)
	if err != nil {
		t.Fatalf("create oversized tar: %v", err)
	}
	writer := tar.NewWriter(file)
	if err := writer.WriteHeader(&tar.Header{Name: "oversized.pem", Mode: 0644, Size: maxArchiveFileSize + 1, Typeflag: tar.TypeReg}); err != nil {
		t.Fatalf("write oversized tar header: %v", err)
	}
	_ = writer.Close()
	if err := file.Close(); err != nil {
		t.Fatalf("close oversized tar: %v", err)
	}
	if err := ExtractTar(tarPath, extractDir); err == nil || !strings.Contains(err.Error(), "大小超过限制") {
		t.Fatalf("oversized archive error = %v", err)
	}
	if _, err := os.Stat(extractDir); !os.IsNotExist(err) {
		t.Fatalf("failed extraction directory should be removed: %v", err)
	}
}
